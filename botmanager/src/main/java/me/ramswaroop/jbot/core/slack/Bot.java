...
import dk.trustworks.botmanager.nlp.ApiAIClient;
...
public abstract class Bot {
            ...
            if (methodWrappers != null) {
                for (MethodWrapper methodWrapper : methodWrappers) {
                    ...
                    if (method.getParameterCount() == 4) {
                        method.invoke(this, session, event, methodWrapper.getNlpResult(), methodWrapper.getMatcher());
                    } else if(method.getParameterCount() == 3) {
                        method.invoke(this, session, event, methodWrapper.getNlpResult());
                    } else {
    ...
    private MethodWrapper getMethodWithMatchingPatternAndFilterUnmatchedMethods(Event event, List<MethodWrapper> methodWrappers) {
        if (methodWrappers != null) {
            ...
            while (listIterator.hasNext()) {
                ...
                if (!StringUtils.isEmpty(pattern) && !StringUtils.isEmpty(result.getAction())) {
                    Pattern p = Pattern.compile(pattern);
                    Matcher m = p.matcher(result.getAction());
                    if (m.find()) {
    ...
    private class MethodWrapper {
        ...
        public Result getNlpResult() {
            return nlpResult;
        }

        public void setNlpResult(Result nlpResult) {
        ...
        public String toString() {
            final StringBuilder sb = new StringBuilder("MethodWrapper{");
